...
# Sample size.
opt <- make_option(c("--studies"), type = "integer", help = "Studies", default = 16)
...
# Base rate.
opt <- make_option(c("--rate"), type = "numeric", help = "Base rate", default = 0.003)
opt_list <- c(opt_list, opt)
...
# Iterations.
opt <- make_option(c("--mc"), type = "integer", help = "MC iterations", default = 10)
opt_list <- c(opt_list, opt)

# Output directory.
opt <- make_option(c("--out"), type = "character", help = "Output stem", default = "Local Results/")
opt_list <- c(opt_list, opt)
